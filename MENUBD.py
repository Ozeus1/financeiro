...
class GerenciadorConfiguracoes2:
    ...
    def criar_interface(self):
        ...
        notebook.add(self.aba_config, text="Configurações")
    ...
    def criar_aba_configuracoes(self):
        frame = ttk.Frame(self.aba_config)
        frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        ttk.Label(frame, text="Configuração de Backup PostgreSQL", font=('Arial', 12, 'bold')).pack(pady=10)
        
        lbl_info = ttk.Label(frame, text="Selecione a pasta onde estão os executáveis do PostgreSQL (pg_dump, pg_restore).\nEx: C:\\Program Files\\PostgreSQL\\16\\bin")
        lbl_info.pack(pady=5)
        
        self.pg_bin_path = tk.StringVar()
        
        row_path = ttk.Frame(frame)
        row_path.pack(fill=tk.X, pady=5)
        
        ttk.Label(row_path, text="Caminho Bin:").pack(side=tk.LEFT)
        ttk.Entry(row_path, textvariable=self.pg_bin_path, width=50, state='readonly').pack(side=tk.LEFT, padx=5)
        ttk.Button(row_path, text="Selecionar Pasta", command=self.selecionar_pasta_postgres).pack(side=tk.LEFT)
        
        ttk.Button(frame, text="Salvar Configuração", command=self.salvar_configuracoes).pack(pady=20)

    def selecionar_pasta_postgres(self):
        path = filedialog.askdirectory(title="Selecione a pasta BIN do PostgreSQL")
        if path:
            self.pg_bin_path.set(path)

    def salvar_configuracoes(self):
        path = self.pg_bin_path.get()
        if not path:
            messagebox.showwarning("Atenção", "Selecione um caminho válido.")
            return
            
        try:
            self.cursor.execute("INSERT OR REPLACE INTO configuracoes (chave, valor) VALUES (?, ?)", ('pg_bin_path', path))
            self.conn.commit()
    ...
    def carregar_configuracoes(self):
        try:
            self.cursor.execute("SELECT valor FROM configuracoes WHERE chave = 'pg_bin_path'")
            res = self.cursor.fetchone()
            if res:
                self.pg_bin_path.set(res[0])
        except sqlite3.Error:
            pass # Tabela pode não existir ainda ou erro de leitura
