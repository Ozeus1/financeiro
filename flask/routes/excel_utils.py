...
def _estilizar_planilha(ws, df, titulo_relatorio, usuario):
    """Aplica cabeçalho do sistema, título, usuário, data de geração e
    estilo à tabela de dados de uma planilha já escrita a partir de
    LINHA_INICIO_DADOS.
    """
...
def gerar_excel_relatorio(df, titulo_relatorio, sheet_name, usuario):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        _estilizar_planilha(writer.sheets[sheet_name], df, titulo_relatorio, usuario)

    output.seek(0)
    return output


def gerar_excel_multiplas_abas(planilhas, titulo_relatorio, usuario):
    """Gera um arquivo Excel em memória com uma ou mais abas, cada uma com
    cabeçalho do sistema, título do relatório, usuário e data/hora de geração.

    `planilhas` é uma lista de tuplas (nome_da_aba, dataframe). Abas com
    dataframe vazio são ignoradas.
    """
    output = BytesIO()
    nao_vazias = [(nome, df) for nome, df in planilhas if not df.empty]
    if not nao_vazias:
        nao_vazias = [planilhas[0]]

    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        for sheet_name, df in nao_vazias:
            df.to_excel(writer, index=False, sheet_name=sheet_name, startrow=LINHA_INICIO_DADOS)
            _estilizar_planilha(writer.sheets[sheet_name], df, titulo_relatorio, usuario)
