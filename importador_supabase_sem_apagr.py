# importador_supabase_final_corrigido.py
...
class SupabaseImporter(tk.Toplevel):
    ...
    def load_config(self):
        ...
        if not self.supabase_url.get():
            self.supabase_url.set("https://gbrktfhxlfqdefuofdpk.supabase.co")
        if not self.supabase_key.get():
            self.supabase_key.set("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imdicmt0Zmh4bGZxZGVmdW9mZHBrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA2MDQwODYsImV4cCI6MjA2NjE4MDA4Nn0.7K8r6i6xjQr__zTE1UY2cWc56O5qPllq9oTe4qdf_BY")
    ...
    def load_data_via_rest_api(self):
        ...
        try:
            ...
            api_url = f"{url}/rest/v1/{table_name}?select=descricao,valor,forma_pagamento,conta_despesa,numero_parcelas,created_at&order=created_at.desc" 
    ...
    def process_loaded_data(self, data):
        ...
        for i, row in enumerate(data):
            ...
            # e como elas foram mapeadas nas funções load_data_via_rest_api/postgresql.
            ...
            if created_at_raw:
                try:
                    # Supabase often returns ISO format with Z for UTC
            ...
            elif isinstance(created_at_raw, datetime): # If it's already a datetime object (from psycopg2)
