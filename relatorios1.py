...
class RelatoriosFinanceirosApp:
    ...
    def conectar_bd(self):
        ...
        try:
            ...
            messagebox.showerror(
                ...
                f"Não foi possível conectar ao 'financas.db': {e}"
            )
            self.root.quit()
            return None
    ...
    def criar_filtros(self):
        ...
        mes_atual_nome = [nome for nome, num in meses.items()
                          if num == datetime.now().strftime('%m')][0]
    ...
    def criar_botoes(self):
        ...
        ttk.Button(
            self.frame_controles,
            text="Por Categoria",
            command=self.gerar_relatorio_categoria
        ).pack(side=tk.LEFT, padx=10)

        ttk.Button(
            self.frame_controles,
            text="Por Meio de Pagamento",
            command=self.gerar_relatorio_meio_pagamento
        ).pack(side=tk.LEFT, padx=5)

        ttk.Button(
            self.frame_controles,
            text="Extrato Detalhado",
            command=self.gerar_relatorio_detalhado
        ).pack(side=tk.LEFT, padx=5)
    ...
    def executar_consulta(self, query, params=None):
        ...
        try:
            ...
            messagebox.showerror(
                "Erro de Consulta",
                f"Ocorreu um erro ao consultar o banco de dados: {e}"
            )
            return []
    ...
    def obter_anos_disponiveis(self):
        ...
        query = (
            f"SELECT DISTINCT strftime('%Y', {self.NOME_COLUNA_DATA}) "
            f"FROM v_despesas_compat "
            f"ORDER BY strftime('%Y', {self.NOME_COLUNA_DATA}) DESC"
        )
    ...
    def gerar_relatorio_categoria(self):
        ...
        self.label_titulo_relatorio.config(
            text=f"Relatório por Categoria - {mes_nome}/{ano}"
        )
        self.limpar_tabela()
    ...
    def gerar_relatorio_meio_pagamento(self):
        ...
        self.label_titulo_relatorio.config(
            text=f"Relatório por Meio de Pagamento - {mes_nome}/{ano}"
        )
        self.limpar_tabela()
    ...
    def gerar_relatorio_detalhado(self):
        ...
        self.label_titulo_relatorio.config(
            text=f"Extrato Detalhado - {mes_nome}/{ano}"
        )
        self.limpar_tabela()
        ...
        query = f"""
            SELECT strftime('%d/%m/%Y', {self.NOME_COLUNA_DATA}),
                   descricao,
                   conta_despesa,
                   printf('%.2f', valor)
            FROM v_despesas_compat
...
def iniciar_relatorios(parent_root):
    """
    Cria uma nova janela (Toplevel) para a interface de relatórios,
    permitindo que ela coexista com a janela principal do sistema.
    """
    relatorios_window = tk.Toplevel(parent_root)
    relatorios_window.title("Relatórios Dinâmicos Simples")
    
    # Agora sem user_id
    app = RelatoriosFinanceirosApp(relatorios_window)
    
    relatorios_window.grab_set()
...
if __name__ == "__main__":
    ...
    # root.mainloop()
    print("Execute através do sistema principal para testar.")
