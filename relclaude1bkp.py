# relclaude.py (Versão corrigida e mais robusta)
...
class RelatoriosFinanceiros:
    ...
    def verificar_conexao_bd(self):
        try:
            ...
            cursor = conn.cursor()
            # Verifica se é tabela ou view
            cursor.execute("SELECT name FROM sqlite_master WHERE (type='table' OR type='view') AND name=?", (self.table_name,))
            if cursor.fetchone() is None:
                messagebox.showerror("Erro de Tabela", f"A tabela/view principal '{self.table_name}' não foi encontrada no banco de dados.")
                conn.close()
                return False
            conn.close()
            return True
    ...
    def gerar_relatorio(self):
        ...
        # Verifica se a função bar_label existe
        if not hasattr(plt.Axes.bar_label, '__call__'):
             messagebox.showerror("Versão Incompatível", "Sua versão da biblioteca 'matplotlib' é muito antiga e não suporta os rótulos nos gráficos.\n\nPor favor, atualize-a com o comando:\npip install --upgrade matplotlib")
             return
    ...
    def relatorio_por_categoria(self, df):
        ...
        # --- ALTERAÇÃO PRINCIPAL ---
        # A forma correta de adicionar rótulos quando se usa o .plot() do pandas
        # é acessar o container de barras diretamente pelo eixo (ax).
        if ax2.containers:
...
if __name__ == "__main__":
    root = tk.Tk()
    # app = RelatoriosFinanceiros(root, user_id=1)
