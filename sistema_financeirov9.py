...
class SistemaFinanceiro:
    def __init__(self, root):
        ...
        arquivo_menu.add_command(label="🔄 Sincronizar Bancos (Flask ↔ Desktop)", command=self.abrir_gerenciador_sync)
...
# ==================================================================
    ...
    def selecionar_item(self, event):
        ...
        # Evita que o clique para ordenação acione a seleção de linha
        if self.tabela.identify_region(event.x, event.y) == 'heading':
            return
...
def main():
    ...
    root.protocol("WM_DELETE_WINDOW", app.on_closing) # Garante que a conexão com o BD seja fechada
    root.mainloop()
